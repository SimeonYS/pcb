...
class PcbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('pcb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pcb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pcb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pcb (date, title, link, content)"
